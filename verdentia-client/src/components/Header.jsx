...
import DialogAddTree from "../shared/dialogAddTree";
...
export default function Header({ currentPage }) {
    ...
    useEffect(() => {
        ...
        }, 1000); // Change the number every 1 second (1000 milliseconds)

        return () => clearInterval(interval); // Cleanup the interval on unmount
    }, []); // Run this effect only on

    const userTypes = ["Login", "Admin", "Staff"]
    const { USER } = useUser();
    ...
    return (
        <>
            <header className="h-[15.5vh] mt-2 flex flex-row">
                <div className="pl-20 py-2 flex flex-col">
                    <h1 className="px-10 text-[3.5vw] font-bold text-slate-800">Verdantia</h1>
                    <h2 className="text-[1.5vw] font-medium text-slate-800">A Tree Management System</h2>
                </div>
                <div className="px-1 pb-4 mt-4 mb-[3.4vh] gap-2 border-2 border-slate-800 rounded-md hover:border-slate-50 hover:bg-slate-800 hover:text-white transition-all duration-300">
                    <h1 className="text-lg font-bold">{USER.uName}</h1>
                    <h2 className="text-md font-medium">{userTypes[USER.userTypeID]}*</h2>
                </div>
                {(currentPage == "treegrid") &&
                    <div className="w-full h-full flex grid grid-cols-5">
                        <span className="col-span-2 "></span>
                        <DialogAddTree TriggerNode={
                            <div className="transform hover:scale-110 transition-all duration-300">
                            <p className=" relative translate-y-[80%] translate-x-[-25%] hover:border-amber-500 flex flex-col self-center border-2 bg-rose-300 rounded-lg w-[10vw] h-[7vh] "></p>
                            <div className="relative translate-y-[-35%]">
                                <div className="h-[10vh] w-[5vw]  col-span-1 pb-7 rounded transition-all duration-300">{TreeSVG[number]}</div>
                                <p className="text-purple-400 font-bold">Add Tree</p>
                            </div>
                        </div>
                        }
                        callback={callback}/>
                        <span></span>
                        <div className="w-[10vw] flex flex-col justify-end self-end">
                            <div className="flex flex-row">
                                <p className="h-[5vh] w-[2.5vw] tree-node pending watering bg-red-100">{TreeSVG[0]}</p>
                                <p className="px-2.5 py-1">Watering</p>
                            </div>
                            <div className="flex flex-row">
                                <p className="h-[5vh] w-[2.5vw] tree-node pending pesticides bg-red-100">{TreeSVG[1]}</p>
                                <p className="px-2 py-1">Pesticides</p>
                            </div>
                            <div className="flex flex-row">
                                <p className="h-[5vh] w-[2.5vw] tree-node pending fertilizing bg-red-100">{TreeSVG[2]}</p>
                                <p className="px-2 py-1">Fertilizing</p>
                            </div>
                        </div>
                    </div>}
            </header>
